...
function filterAndCopyRows() {
    ...
    var range = templateSheet.getRange(10, 1, lastRow - 9, lastColumn);
    ...
    var filteredFontColors = [];
    var filteredFontStyles = [];
    var filteredFontWeights = [];
    ...
    for (var row = 0; row < values.length; row++) {
        ...
        if (cellValue !== finalResultMenu1 && cellValue !== allResultMenu1 || backgrounds[row][0] === "#00AEEF") {
            ...
            filteredFontColors.push(fontColors[row]);
            filteredFontStyles.push(fontStyles[row]);
            filteredFontWeights.push(fontWeights[row]);
        }
    }
    ...
    if (filteredRows.length > 0) {
        ...
        targetRange.setFontColors(filteredFontColors);
        targetRange.setFontStyles(filteredFontStyles);
        targetRange.setFontWeights(filteredFontWeights);

        // Встановлюємо межі вручну
        for (var row = 0; row < filteredRows.length; row++) {
            for (var col = 0; col < lastColumn; col++) {
                var cell = resultSheet.getRange(row + 1, col + 1);
                cell.setBorder(true, true, true, true, false, false);
            }
        }

        Logger.log("✅ Фільтровані рядки успішно скопійовані з повним форматуванням!");
    } else {
